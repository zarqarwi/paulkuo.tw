...
Perplexity 回答问题的时候会引用来源，ChatGPT 的浏览模式会抓取网站结构化数据，Claude 可以通过 llms.txt 理解网站。这意味着什么？意味着一个网站真正的任务，正在从"被人看到"转向"被 AI 正确理解"。SEO 优化的是搜索引擎可见度；AEO（Answer Engine Optimization）——也有人叫 GEO（Generative Engine Optimization）——优化的是被 AI 正确摘要、正确引用、正确链接的能力，点击率只是副产品。
...
做很多修改可以让分数一直上去，但那只是反馈循环在说谎。可持续的优化需要的是一套能区分有效信号与无效波动的研究制度。
...
paulkuo.tw 是我跟 AI 共同构建知识的现场——文章、代码和这套优化循环，都是那个现场的一部分。
