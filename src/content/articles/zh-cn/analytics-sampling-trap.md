...
subtitle: "Cloudflare Analytics 的取样陷阱，网站的人头数该怎么算？"
description: "从发现 Cloudflare Web Analytics API 回传 visits=0 开始，完整记录排查过程、两套分析系统的差异、adaptive sampling，自建 beacon 的架构决策。"
abstract: |
...
readingTime: 8
# === AI / Machine 专用栏位 ===
thesis: "不是只有流量型网站才需要在意流量数据。很多 B2B 网站本来就不是靠大量访客取胜，很多个人网站也不是靠追逐时事与风潮生存；但只要你在经营品牌、内容或转换路径，就需要可信的分析基础。Cloudflare 的 GraphQL Analytics API 在日访客低于 500 的网站上，容易出现严重取样失真；对品牌网站来说，若没有自建 visit beacon，就很难拿到真正可用的流量数据。"
domain_bridge: "网站工程 × 数据品质 × 个人品牌经营"
confidence: high
...
related_entities:
  ...
  - name: Imperva
    type: Organization
reading_context: |
  适合正在经营个人网站或公司官网的技术人员、行销人员、独立开发者。不需要深度 Cloudflare 知识，但需要理解「为什么流量数据准确很重要」。
---
...
当数据落差过大时，比较直接的办法就是自己数——自己建立精确的访问计数机制。Cloudflare 本身值得信任，问题在于取样在流量规模还小的时候，先天就容易失准。随着受众规模扩大，取样精度自然提升；但偏偏就在流量还在成长的阶段，每一个数字都攸关方向判断。

网站经营不能只靠感觉。五分钟：拿 API 数字跟 Dashboard 比对。如果差距超出合理误差，你就知道接下来该怎么做了。