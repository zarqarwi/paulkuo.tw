...
Perplexity は質問に答える際にソースを引用する。ChatGPT のブラウジングモードはウェブサイトの構造化データを読み取る。Claude は llms.txt を通じてウェブサイトを理解できる。これは何を意味するか？ウェブサイトの本当の使命が「人間に見てもらう」から「AI に正しく理解してもらう」へとシフトしているということだ。SEO が最適化するのは検索エンジンの可視性だ。AEO（Answer Engine Optimization）——GEO（Generative Engine Optimization）とも呼ばれる——が最適化するのは、AI に正確に要約され、引用され、リンクされる能力であり、クリック率は副産物に過ぎない。
...
変更を重ねればスコアは上がり続ける——しかしそれはフィードバックループが嘘をついているだけだ。持続可能な最適化に必要なのは、有効なシグナルと無効な波動を区別できる研究制度を構築することだ。
...
paulkuo.tw は AI とともに知識を共構築する現場だ——記事も、コードも、この最適化ループも、すべてその現場の一部である。
