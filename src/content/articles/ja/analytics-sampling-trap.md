...
だから正しい問いに変わった：完全な「読者オーディエンス」をどう見るか？
...
落差が大きすぎる場合、一番シンプルな対処法は自分で数えることだ——自前の正確な訪問カウント機構を構築する。Cloudflare自体は信頼に値する。問題はサンプリングがトラフィック規模がまだ小さい時に先天的に不正確になりやすい点だ。受け手規模が拡大するにつれてサンプリング精度は自然に上がる——しかしまさにトラフィックが成長中の段階こそ、一つひとつの数字が方向判断に直結する。

ウェブサイト運営は感覚だけに頼れない。5分：APIの数字とDashboardを比較する。差が合理的な誤差を超えていれば、次に何をすべきか自ずとわかる。