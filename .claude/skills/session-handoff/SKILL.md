...
> C 層是針對 Chat 能力邊界裁切的 adaptation，版本線獨立（C 層 v1.0 對齊 A 層 v5.6）。
> Git snapshot：`docs/governance/c-layer-snapshot.md`。
> 同步協議待 Chat 立法（H1 ADR）。
...
### -1. 同步 skill 到 Cowork cache（v5.7 新增）

Cowork 讀的 skill 在 `/var/folders/.../T/claude-hostloop-plugins/*/skills/`（cache），
不是 repo `.claude/skills/`（A 層 SSoT）。兩邊不會自動同步，skill 更新後 cache 會停留在舊版。

**每次 Cowork 開場先跑**（透過 `Control your Mac` osascript）：

```applescript
do shell script "bash ~/Desktop/01_專案進行中/paulkuo.tw/scripts/sync-skills-to-cowork.sh 2>&1"
```

腳本會：
- 把 repo 四個 mirror skill（paulkuo-writing / paulkuo-social / formosa-feedback / organize-downloads）直推 cache
- 比對 session-handoff 的 A 層版本號與 C 層「對齊 A 層」標記，不對齊跳警告（C 層需人工裁切，不自動改）

輸出若出現 `🔄` 表示有 skill 被更新了，下次開場讀到的就是新版。
若出現 `🚨 C 層沒對齊 A 層` → 提醒 Paul 手動更新 C 層 adaptation。

> **為什麼放 -1 而不是 0：** skill 同步要在其他盤點步驟之前完成，否則本次 session 讀到的可能仍是舊 skill 內容。
> 觸發事件：2026-04-24 Paul 在 Cowork 請 Claude 改 paulkuo-writing 第四幕，Claude 讀到 cache v2.4（舊版），差點把 Paul 已手動精簡的 SDTI/CircleFlow 段改回冗長版。根因：cache 停在 v2.4，A 層已到 v2.7。
