...
| v2.7 | 2026-04-24 | 第四幕擴大定義：Paul 聲音不一定要靠自我揭露句型，專業細節（別人寫不出來的產業知識、數字、運作邏輯）也算。提到個人經驗時用「在循環經濟產業做商務開發/整合的過程」這種描述，不直接寫公司名（SDTI / CircleFlow 等）。觸發事件：2026-04-24 對話中 Paul 指出「我在 SDTI 時⋯」已變成儀式性填空段落。 |
...
4. **Paul 聲音接入**：可以是個人經歷，也可以是專業細節（別人寫不出來的產業知識、數字、運作邏輯）。提到個人經驗時用「在循環經濟產業做商務開發/整合的過程」「在教會服事的時候」這種描述，不直接寫公司名（SDTI / CircleFlow 等）。每篇要有一處「非你不能寫」的密度，但不一定要靠自我揭露句型。
