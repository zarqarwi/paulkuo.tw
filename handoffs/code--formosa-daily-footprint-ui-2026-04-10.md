...
Cowork 剛推了這份 handoff 到 main（commit `1f324df`），不 pull 會衝突。
...
**位置**：`src/i18n/translations/` 下四個語系檔（zh-Hant、zh-Hans、en、ja）
...
| 項目 | 原因 |
|------|------|
| `handleFormosaDailyReport` handler | 完整且正確，接收欄位和 upsert 邏輯都不需要改 |
| `formosa_daily_reports` 表 schema | 已有所有需要的欄位 |
| `handleFormosaSubmit`（問卷提交） | Q1-Q10 流程不動 |
| GWP_FACTORS 碳排係數 | 不動（water: 0.10974, hotel: 8.85） |
| 等級計算 / 打卡邏輯 | 完全不碰 |
...
# 測試 admin/carbon 撈得到 daily_reports 的資料
...
# ✅ 預期：water_bottles_total ≥ 3, hotel_nights_total ≥ 1（至少有剛才的測試資料）

# 清理測試資料（如果有清理 endpoint 的話，沒有就跳過，量級可忽略）
```

### 4-2. 前端頁面測試

用瀏覽器開 Tracker 頁面（部署後）：

1. **登入** → 打卡 → 確認打卡成功區塊下方出現「🌱 今日善足跡」收合面板
...
開 Dashboard 頁面，確認碳足跡區塊的「瓶裝水」和「住宿」數字不再是 0（前提：有任何 daily_report 資料）。
...
## 完成日誌（最新在上）
- {HH:MM} admin/carbon SQL 改撈 formosa_daily_reports 表 ({commit hash}) Code
- {HH:MM} i18n dailyReport keys 精簡，移除交通相關（4 語系）({commit hash}) Code
- {HH:MM} 恢復今日善足跡選填 UI：水瓶 + 住宿收合面板 ({commit hash}) Code
...
Step 0：偵察（6 項確認）
    ↓ 全部 OK
Step 1：前端加回善足跡面板（_TrackerPage.astro）
    ↓
Step 2：i18n 精簡 dailyReport keys（4 語系）
    ↓
