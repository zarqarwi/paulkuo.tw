...
export async function handleScorecardEvaluate(request, env) {
  ...
  const { projectName, projectDesc, stage, inputContent, githubMeta, lang } = body;
...
${inputContent.slice(0, 8000)}`;
  ...
  if (githubMeta) {
    userMsg += `\n\n## GitHub 結構化數據
Stars: ${githubMeta.stars ?? 'N/A'}
Contributors: ${githubMeta.contributors ?? 'N/A'}
Last commit: ${githubMeta.lastCommit ?? 'N/A'}
Open issues: ${githubMeta.openIssues ?? 'N/A'}
Languages: ${githubMeta.languages ?? 'N/A'}
License: ${githubMeta.license ?? 'N/A'}
Dependents: ${githubMeta.dependents ?? 'N/A'}`;
  }
