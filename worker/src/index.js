...
import { fetchDailyVisitors, handleVisitors, handleAnalytics, handleAnalyticsBeacon, handleVisitBeacon, fetchAnalyticsOverview, fetchRumAnalytics, fetchDurationAnalytics, fetchZoneUniqueVisitors, aggregateBotAnalytics, handleBotBackfill, handleReclassify } from './visitors.js';
...
async function handleRequest(request, env, ctx) {
  ...
  if (path.startsWith('/api/formosa/') && method === 'POST'
      && path !== '/api/formosa/upload'
      && path !== '/api/formosa/feedback-upload') {
  ...
  if (path === '/api/formosa/auth/role' && method === 'GET') return handleFormosaAuthRole(request, env);
  ...
  if (path === '/api/formosa/admin/clusters' && method === 'GET') return handleFormosaAdminClusters(request, env);
  if (path === '/api/formosa/admin/status') return handleFormosaAdminStatus(request, env);
  if (path === '/api/formosa/admin/roles') return handleFormosaAdminRoles(request, env);
  ...
  if (path === '/api/formosa/line-usage' && method === 'GET') return handleFormosaLineUsage(request, env);
  if (path === '/api/formosa/feedback/status' && method === 'GET') return handleFormosaFeedbackPublicStatus(request, env);
  if (path.match(/^\/api\/formosa\/feedback\/\d+$/) && method === 'PATCH') return handleFormosaFeedbackUpdate(request, env);
  ...
  if (path === '/api/wiki/youtube-ingest' && method === 'POST') return handleYoutubeIngest(request, env);
...
async function handleScheduled(event, env) {
  ...
  if (event.cron !== '*/5 * * * *') {
    ...
    try { await aggregateBotAnalytics(env); console.log('Cron: bot analytics aggregated'); } catch (e) { console.error('Cron: bot analytics FAILED:', e.message); }
    ...
    try {
      ...
      if (!gscLast || (Date.now() - parseInt(gscLast)) >= 86400000) {
        const r = await fetchGscData(env);
        console.log('Cron: GSC fetch', JSON.stringify(r));
      }
    } catch (e) { console.error('Cron: GSC fetch FAILED:', e.message); }
    // YouTube channel scan (daily, runs on hourly cron but self-limits to once/day)
    try {
      const ytLast = await env.TICKER_KV.get('youtube:last_scan');
      if (!ytLast || (Date.now() - new Date(ytLast).getTime()) >= 86400000) {
        const r = await handleYoutubeChannelScan(env);
        console.log('Cron: youtube scan', JSON.stringify(r));
      }
    } catch (e) { console.error('Cron: youtube scan FAILED:', e.message); }
  }
}
